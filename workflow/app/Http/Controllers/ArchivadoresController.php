...
class ArchivadoresController extends Controller {
    ...
    public function GetList() {
        ...
        if (!$AC->CheckAccess(['admin/archivadores'])) return $AC->NoAccess();
        ...
        foreach ($itemList as $item) {
            $response[] = [
                'id' => $item->id,
                'nombre' => $item->nombre,
                'fecha' => Carbon::parse($item->createdAt)->format('d-m-Y'),
    ...
    public function GetData(Request $request) {
        ...
        $itemList = ArchivadorDetalle::all();

        $archivadorDetalle = [];

        foreach ($itemList as $archivador) {
            $archivadorDetalle[$archivador->id] = $archivador->toArray();
        }

        // dd($archivadorDetalle);

        $data = [];
        $dataRespuestas = CotizacionDetalle::where('archivadorId', $id)->limit(1000)->get();

        // dd($dataRespuestas);

        foreach ($dataRespuestas as $respuesta) {
            $data[] = [
                'id' => $respuesta->cotizacionId,
                'nombre' => (!empty($archivadorDetalle[$respuesta->archivadorId]) ? $archivadorDetalle[$respuesta->archivadorId]['nombre'] : ''),
                'desde' => $respuesta->campo,
                'valor' => $respuesta->valorLong,
            ];
        }

        if (!empty($itemList)) {
            return $this->ResponseSuccess('Información obtenida con éxito', $data);
        }
        else {
            return $this->ResponseError('Error al obtener aplicaciones');
        }
