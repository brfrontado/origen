...
class EtapasController extends Controller {
    ...
    public function generatePDF(Request $request) {
        ini_set('max_execution_time', 300);
        $validateForm = Validator::make($request->all(), ['cotizacionId' => 'required',]);
        if ($validateForm->fails()) {
            $errores = $validateForm->errors()->keys();
            return $this->ResponseError('CLIENT-AFdd5ds834', 'Faltan Campos ' . implode(',', $errores));
        }
            ...
            $tareaHandler = new TareaController();
            $resultado = $tareaHandler->CalcularPasos($request, true);

            $htmlContent = '';
            if (!empty($resultado['actual']['salidaReplaced'])) {
                $htmlContent = $resultado['actual']['salidaReplaced'];
            }
            ...
            $arrFinal['pdf'] = $pdfBase64;

            return $this->ResponseSuccess('PDF Generado con éxito', $arrFinal);
        }
    }
